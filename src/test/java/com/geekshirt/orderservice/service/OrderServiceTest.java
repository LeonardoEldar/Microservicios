...
import static com.geekshirt.orderservice.util.OrderServiceDataTestUtils.getMockOrderRequest;
import static com.geekshirt.orderservice.util.OrderServiceDataTestUtils.getMockPayment;
import static com.geekshirt.orderservice.utils.OrderPaymentStatus.APPROVED;
import static com.geekshirt.orderservice.utils.OrderPaymentStatus.DENIED;
import static com.geekshirt.orderservice.utils.OrderStatus.PENDING;
import static org.mockito.ArgumentMatchers.*;

//Pruebas unitarias, no se utilizan los contenedores de Spring
...
public class OrderServiceTest {
    ...
    @BeforeEach
    public void init() {
        AccountDto mockAccount = OrderServiceDataTestUtils.getMockAccount();
    ...
    public void shouldThrowIncorrectExceptionWhenOrderItemsAreNull() {
        ...
        Assertions.assertEquals(ExceptionMessagesEnum.EMPTY_ORDER_ITEM.getValue(), incorrectException.getMessage());
    }
    ...
    public void shouldThrowAccountNotFoundWhenAccountDoesNotExists() {
        OrderRequest orderRequest = getMockOrderRequest("12345678");
    ...
    public void shouldThrowPaymentNotAcceptedExceptionWhenPaymentIsDenied() {
        ...
        //AccountDto mockAccount = getMockAccount(orderRequest.getAccountId());
        //Mockito.doReturn(Optional.of(mockAccount)).when(customerClient).findAccountById(anyString());

        Mockito.when(paymentService.processPayment(any(), any())).thenReturn(getMockPayment(orderRequest.getAccountId(), DENIED));
    ...
    public void shouldReturnPendingOrderWhenCreateOrderIsCalled() throws PaymentNotAcceptedException {
        ...
        Mockito.when(paymentService.processPayment(any(), any())).thenReturn(OrderServiceDataTestUtils
                .getMockPayment(orderRequest.getAccountId(), APPROVED));
        ...
        Assertions.assertEquals(APPROVED, order.getPaymentStatus());
        Assertions.assertNotNull(order.getTransactionDate());

        /*
        Otra forma de realizar las validaciones
